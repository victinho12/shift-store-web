...
async function alterarCart(req, res ,next) {
  try{
    ...
    if (quantidade < 0) throw new AppError("Quantidade de alteração invalida, selecione um valor inteiro positivo", 400, "QUANTIDADE_INDISPONIVEL");
    const cart = await pool.query(`UPDATE PUBLIC.carrinho_item set quantidade = $1 where id = $2 RETURNING id`,[quantidade,id]);
    if(cart.rowCount === 0) throw new AppError("Não foi possivel alterar valor", 400, "VALOR_INVALIDO", `O id do carrinho ${id}, quantidade: ${quantidade}`);
